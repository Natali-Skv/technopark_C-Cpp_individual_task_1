...
TEST(MULTIPLY_MATRICES, MULTIPLY_INVALID_ARGS) {
    ...
    // product matrix is not empty
    EXPECT_NE(multiply_matrices(&mtrx_2x1, &mtrx_1x2, &mtrx_2x2), 0);
...
TEST(MULTIPLY_MATRICES, CALLOC_MAX_SIZE_INTEGERS) {
    Matrix_t mtrx_lhs = {SIZE_MAX, 1u, NULL};
    Matrix_t mtrx_rhs = {1u, 1u, NULL};
    Matrix_t mtrx_product = {0u, 0u, NULL};
    ASSERT_TRUE(mtrx_lhs.elems = calloc_mtrx(1u, 1u));
    ASSERT_TRUE(mtrx_rhs.elems = calloc_mtrx(1u, 1u));

    EXPECT_NE(multiply_matrices(&mtrx_lhs, &mtrx_rhs, &mtrx_product), 0);

    free_mtrx(mtrx_lhs.elems, 1u);
    free_mtrx(mtrx_rhs.elems, 1u);
}
