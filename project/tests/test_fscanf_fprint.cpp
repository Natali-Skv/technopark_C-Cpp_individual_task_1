...
TEST(FSCANF_FPRINTF, NULL_ARGS) {
    ...
    EXPECT_NE(fprintf_mtrx(NULL, &mtrx), 0);
    EXPECT_NE(fprintf_mtrx(stdin, &mtrx), 0);
}

TEST(FSCANF_FPRINTF, ERANGE) {
    FILE *fin = fopen(PROJECT_PATH "tests/input_emulator/fscanf_fprintf/erange", "r");
    Matrix_t mtrx = {0u, 0u, NULL};
    EXPECT_NE(fscanf_mtrx_sizes(fin, &mtrx), 0);
    EXPECT_NE(fscanf_mtrx_sizes(fin, &mtrx), 0);
    fclose(fin);
}
